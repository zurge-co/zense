...
/**
 * The change under `line` (modified-side line number). Pure deletions cover
 * a single context line on the modified side. When no change contains the
 * line, return the nearest one so a right-click slightly off a chunk still
 * explains something useful; undefined only when there are no changes.
 */
export function findChangeAtLine(
  ...
  for (const ch of changes) {
    const mod = toInclusiveRange(ch.modifiedStartLineNumber, ch.modifiedEndLineNumber);
    // An empty modified range still "covers" its insertion point (start-1)
    // — the line the cursor sits on when viewing a deletion.
    const coverStart = Math.min(mod.start, ch.modifiedStartLineNumber);
    const coverEnd = Math.max(mod.end, ch.modifiedStartLineNumber);
